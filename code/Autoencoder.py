...
class Bottle2neck(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.conv1(x) #   104,32,32
        out = self.bn1(out) #   104,32,32
        out = self.relu(out)
...
class Res2Net(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.conv1(x)   # (64 64 64) #(500,500)
        x = self.bn1(x)     
        x = self.relu(x)
        x = self.maxpool(x) # (64,32,32) #(250,250)

        x_layer1 = self.layer1(x)                   #(256,32,32) bottle2neckblock -> 3개
        x_layer2 = self.layer2(x_layer1)            #(512,16,16) bottle2neckblock -> 4개
        x = self.layer3(x_layer2)                   #(1024,8,8)  bottle2neckblock -> 23개
...
class Dehaze(nn.Module):
    def __init__(self):
        ...
        self.encoder.state_dict(model_dict)
        ...
        self.tail = nn.Sequential(nn.ReflectionPad2d(3), nn.Conv2d(28, 1, kernel_size=7, padding=0), nn.Tanh())
        self.conv3 = nn.Conv2d(1024,64,kernel_size=1,stride=1)
        self.conv4 = nn.Conv2d(64,1024,kernel_size=1,stride=1)
        self.inplanes = 64

    def forward(self, input):

        x,x_layer1,x_layer2 = self.encoder(input) #512,56,64 
    
        # x - (1024,8,8)
        x_mid = self.mid_conv(x)    # (1024,8,8)
        x_mid = self.conv3(x_mid)
        data = x_mid
        x_mid = self.conv4(x_mid)
        x = self.up_block1(x_mid)   # (256,16,16)
        x = self.attention1(x)      # (256,16,16)

        x = torch.cat((x, x_layer2), 1) #(768,16,16)
        x = self.up_block1(x)           #(192,32,32)
        x = self.attention2(x)          #(192,32,32)
        
        x = torch.cat((x, x_layer1), 1) #(449,32,32)
        x = self.up_block1(x)           #(112,64,64)
        x = self.up_block1(x)           #(28,128,128)
        
        x = self.enhancer(x)            #(28,128,128)
        out = self.tail(x)              #(3,128,128)
