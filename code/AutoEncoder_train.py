...
BASE_PATH = 'E:\\new_model(Unet + SRGAN)\\SRGAN + Teachernetwork\\result-loss_0.001\\'
...
netE.load_state_dict(torch.load("E:\\new_model(Unet + SRGAN)\\HR_AutoEncoder_pretrained\\KTDN_teachnetwork_checkpoint_200\\checkpoint_KTDN\\checkpoint\\epoch_200_ip102data_netE.pth"))
...
for epoch in range(START_EPOCH + 1, NUM_EPOCHS + 1):
    ...
    for LR_image, HR_image in train_loader:
        ...
        x7,SR_image             = netG(LR_image)
        SR_hr_score             = netD(SR_image).mean()
        out,middle_feature      = netE(HR_image)
